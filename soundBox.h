...
{
public:
	soundBox(wstring sOutputDevice, unsigned int nSampleRate = 44100, unsigned int nChannels = 1, unsigned int nBlocks = 8, unsigned int nBlockSamples = 512)
	{
	...
	bool Create(wstring sOutputDevice, unsigned int nSampleRate = 44100, unsigned int nChannels = 1, unsigned int nBlocks = 8, unsigned int nBlockSamples = 512)
	{
		...
		m_thread = thread(&soundBox::MainThread, this);
...
private:
	...
	{
		((soundBox*)dwInstance)->waveOutProc(hWaveOut, uMsg, dwParam1, dwParam2);
	}
